#!/usr/bin/env python3
"""Build and validate the vision GitHub Release assets.

The release payload is deliberately sourced from one directory only:
``vision/``.  The script uses only Python's standard library so it can
...
PACKAGE_DIR_NAME = "vision"
VERSION_FILE = "operate/VERSION"
USER_AGENTS_FILE = "operate/user_AGENTS.md"
VERSION_MARKER = re.compile(r"vision-version:\s*([^\s<]+)")
IDENTIFIER = re.compile(r"^[0-9A-Za-z-]+$")
USER_ID_MARKER = "<!-- vision-user-id: IyedBecheikh/vision -->"
USER_MANAGED_START = "<!-- vision-user-managed-start -->"
USER_MANAGED_END = "<!-- vision-user-managed-end -->"
...
def _validate_user_agents_text(text: str, version_text: str) -> None:
    ...
    if f"<!-- vision-version: {version_text} -->" not in text:
...
def _verify_member_names(names: Iterable[str]) -> list[str]:
    ...
    required = {
        ...
        f"{PACKAGE_DIR_NAME}/operate/config.md",
        ...
        f"{PACKAGE_DIR_NAME}/runtime/commands.py",
        ...
        f"{PACKAGE_DIR_NAME}/runtime/platform_lifecycle.py",
        ...
        f"{PACKAGE_DIR_NAME}/runtime/session_usage.py",
    ...
    required.update(
        f"{PACKAGE_DIR_NAME}/workers/{worker}.md" for worker in BUILTIN_WORKERS
    )
...
def verify_archive(archive_path: Path, expected_version: SemVer | None = None) -> str:
    if archive_path.suffix == ".zip":
        with zipfile.ZipFile(archive_path) as archive:
            ...
            with tempfile.TemporaryDirectory(prefix="vision-verify-") as temporary:
