...
PROJECT_ID = "<!-- vision-id: IyedBecheikh/vision -->"
USER_ID = "<!-- vision-user-id: IyedBecheikh/vision -->"
WORKER_MARKER = re.compile(r"^# vision-worker: ([A-Za-z0-9_-]+)$", re.MULTILINE)
SKILL_MARKER = re.compile(
    r"^<!-- vision-skill: ([a-z0-9-]+) -->$", re.MULTILINE
)
...
class PackageLayout:
    ...
    def validate(self, *, allow_legacy: bool = False) -> None:
        ...
        if USER_ID not in user_agents_text:
            raise ValidationError("package user_AGENTS.md marker is missing")
        if f"<!-- vision-version: {version} -->" not in user_agents_text:
        ...
        if not allow_legacy:
            ...
            missing_workers = [
                role for role in sorted(BUILTIN_WORKERS) if not (self.workers / f"{role}.md").is_file()
            ]
            if missing_workers:
                raise ValidationError(
                    f"package canonical worker instructions missing: {missing_workers}"
                )
        required_docs = {
            "project_overview.md",
        ...
        if not templates and not (self.workers.is_dir() and any(self.workers.glob("*.md"))):
            raise ValidationError("package has no worker templates")
        if not allow_legacy and templates and templates != BUILTIN_WORKERS:
            raise ValidationError(
    ...
    def workers(self) -> Path:
        """Canonical, platform-neutral worker instruction store."""
...
class ProjectPaths:
    ...
    def source_dir(self) -> Path:
        ...
        return self.root / "Vision_Workflow"
